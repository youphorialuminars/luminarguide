...
import { siteConfig, gradeBands, getGradeBand, type GradeBand, type Pillar } from '@/lib/siteConfig';
 ...
 * launcher button. variant="inline" is the same game, available for reuse
 * elsewhere if needed. Kept here (not its own file) for the same reason as
...
export function CollaboratorInterestForm() {
  ...
  const handleChange = (
    ...
    // There's no backend to actually receive an uploaded file yet (same reason
  // this whole form falls back to mailto instead of a real submission) — a
  // browser can't attach a file to a mailto draft for security reasons. So
  // this only captures the file's name, and the mailto body reminds the
  // visitor to attach it themselves once their email client opens.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const subject = `${form.role} interest — LuminarGuide`;
        const body = [
          `Name: ${form.name}`,
          `Role: ${form.role}`,
          ...
          documentName ? `\nDon't forget to attach: ${documentName}` : '',
          '',
          'Message:',
          form.message,
        ].join('\n');
    ...
    <div className="bg-card border border-border rounded-2xl p-7 max-w-2xl mx-auto">
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
              ...
              <p className="text-xs text-muted-foreground">
                Your email client will open blank — we'll remind you in the pre-filled message to attach{' '}
                {documentName ? <span className="font-600 text-foreground">{documentName}</span> : 'your file'} before sending.
              </p>
            </div>
        <div className="flex flex-col gap-1.5">
        ...
        <button type="submit" className="btn-primary justify-center text-center">
          {submitted ? 'Opening your email client…' : 'Share My Interest'}
        </button>
        <p className="text-xs text-muted-foreground text-center">
          This opens your email client with your details filled in. If nothing opens, email us directly at{' '}
    ...
    <>
      <section className="py-20 bg-background">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <h1 className="text-section-heading text-foreground mb-5">Real experiences from the Luminar's Guide community.</h1>
        ...
        <div className="bg-card border border-border rounded-2xl p-7 max-w-2xl mx-auto">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Your email client will open blank — we'll remind you in the pre-filled message to attach{' '}
                {photoName ? <span className="font-600 text-foreground">{photoName}</span> : 'your photo'} before sending.
              </p>
            </div>
            <a href={mailtoHref} className="btn-primary justify-center text-center mt-1">
              Send Your Story
            </a>
          </div>
        </div>
